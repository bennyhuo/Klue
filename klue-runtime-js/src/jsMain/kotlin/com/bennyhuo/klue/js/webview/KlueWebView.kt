...
object KlueWebView: Klue {
    ...
    override fun <T> callNative(className: String, functionName: String, args: String): Promise<T> {
        return Promise<T> { resolve, reject ->
            callNative(className, functionName, args, object : BridgeCallback<T> {
                override fun onSuccess(returnValue: T) {
                    println("onSuccess: ${JSON.stringify(returnValue)}")
                    resolve(returnValue)
                }
    ...
    private fun <T> callNative(
        className: String,
        functionName: String,
        args: String,
        bridgeCallback: BridgeCallback<T>
    ) {
        ...
        val timeoutHandle = window.setTimeout({
            ...
            bridgeCallback.onError(-1, "Timeout")
        }, 5000)
        ...
        window.asDynamic()[functionInfo.callback] = { returnValue: dynamic ->
            ...
            if (returnValue.code == 0) {
                bridgeCallback.onSuccess(JSON.parse(returnValue.data))
            } else {
                bridgeCallback.onError(returnValue.code, returnValue.data)
            }
        }
